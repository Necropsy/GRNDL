...
std::set<std::string> GRN::fullGRN(){

    this->atratores.clear();

    unsigned int *period_vet = nullptr;
    unsigned int *transient_vet = nullptr;
    
    period_vet = (unsigned int *)malloc(sizeof(unsigned int)*this->NUM_STATES);
    transient_vet = (unsigned int *)malloc(sizeof(unsigned int)*this->NUM_STATES);
    
    if(period_vet == nullptr || transient_vet == nullptr){
        exit(255);
    }

    runGRN(0,this->NUM_STATES,period_vet,transient_vet);

    free(period_vet);
    free(transient_vet); 
    return this->atratores;
}

void GRN::runGRN(int inicio, int fim, unsigned int * period_vet, unsigned int * transient_vet) {
    bool s0[this->NUM_NOS];
    bool s1[this->NUM_NOS];
    ...
    for (unsigned long long int i = inicio; i < fim; i++) {
        initialState(i, s0, s1, this->NUM_NOS);
        do {
            passo(s0);
            passo(s1);
            passo(s1);
            ...
            if(equals(s0, s1, this->NUM_NOS)){
            	atratores.insert(boolArraytoString(s0, this->NUM_NOS));
            }

        } while (!equals(s0, s1, this->NUM_NOS));
        do {
            passo(s1);
            period++;
        } while (!equals(s0, s1, this->NUM_NOS));
...
std::set<std::string> GRN::atrator(std::string vet) {
    bool s0[this->NUM_NOS];
    bool s1[this->NUM_NOS];
    this->atratores.clear();

    for (int i = 0; i < this->NUM_NOS; i++) {
        if(vet[i] == 0){
            s0[i] = 0;   
        }else{
            s0[i] = 1;
    ...
    #pragma omp parallel for schedule(static) private(s0,s1)
    do {
        passo(s0);
        passo(s1);
        passo(s1);
        if(equals(s0, s1, this->NUM_NOS)){
            this->atratores.insert(boolArraytoString(s0, this->NUM_NOS));
        }
    } while (!equals(s0, s1, this->NUM_NOS));
    return this->atratores;
}

void GRN::initialState(unsigned long int valor, bool *vet1, bool *vet2, int size) {
...
bool GRN::equals(bool *vet1, bool *vet2, int size) {
    for (int i = 0; i < size; i++) {
        if (vet1[i] != vet2[i]) {
            return false;
        }
...
std::string GRN::boolArraytoString(bool *vet, int size) {
    std::string out;
...
void GRN::passo (bool * aux){

    bool vet[this->NUM_NOS];
